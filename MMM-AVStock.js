...
var header = ["symbol", "price", "close", "change", "changeP", "volume"];
var headerTitle = ["Symbol", "Cur.Price", "Prev.Close", "CHG", "CHG%", "Volume"];
...
Module.register("MMM-AVStock", {
    ...
    update: function(stock) {
        switch (this.config.mode) {
        case "table":
            this.drawTable(this.stocks[stock]);
            break;
        case "ticker":
            this.drawTicker(this.stocks[stock]);
            break;
        case "series":
            this.drawSeries(this.stocks[stock]);
            break;
        default:
            console.error("[MMM-AVStock] Unknown mode. Please check config.");
        }
    },
    ...
    changeV = series[0].close - series[1].close;
    var changePerc = (changeV / s.close) * 100;
    ...
    change.innerHTML = changeV.toFixed(this.config.decimals) + " / " + changePerc.toFixed(this.config.decimals) + "%";
